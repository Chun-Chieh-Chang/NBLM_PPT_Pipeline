...
<p align="center">
  ...
  <a href="[Local URL]">Flip through all 17 examples online →</a> · <a href="./examples/"><code>examples/</code> directory</a> (17 projects, 229 pages) · <a href="./docs/why-ppt-master.md">Why PPT Master?</a></sub>
</p>
...
**Option A — Download ZIP** (no Git required): click **Code → Download ZIP** on the [GitHub page]([Local Repository]), then unzip.
...
**A) AI generation & Guizang HTML Compilation** — `image_gen.py` and `guizang_pipeline.py`. Set the provider's `*_API_KEY` (`AGNES_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, etc.), and the pipeline calls it automatically. When using Agnes AI, the system automatically routes HTML text compilation to `agnes-2.0-flash` and image generation to `agnes-image-2.1-flash`. Run `python3 skills/ppt-master/scripts/image_gen.py --list-backends` for the full image backend list.
